...
public:
	...
	void calc_transform(rs2_pose& pose_data, float mat[16])
	{
		...
		//mat[3],mat[7],mat[11] is Translation vector while remaning is Rotation matrix.
		mat[0] = -(1 - 2 * q.y*q.y - 2 * q.z*q.z); mat[4] = 2 * q.x*q.y - 2 * q.z*q.w;     mat[8] = -(2 * q.x*q.z + 2 * q.y*q.w);
		mat[1] = -(2 * q.x*q.y + 2 * q.z*q.w);     mat[5] = 1 - 2 * q.x*q.x - 2 * q.z*q.z; mat[9] = -(2 * q.y*q.z - 2 * q.x*q.w);
		mat[2] = -(2 * q.x*q.z - 2 * q.y*q.w);     mat[6] = 2 * q.y*q.z + 2 * q.x*q.w;     mat[10] = -(1 - 2 * q.x*q.x - 2 * q.y*q.y);
		mat[3] = t.x;                              mat[7] = t.y;                           mat[11] = t.z;
	}
...
// In this example, we show how to track the camera's motion using a T265 device, **merged rs-trajectory with rs-savetodisk**
int main(int argc, char * argv[]) try
{
	...
	// **Connecting code and camera which is used with the help of pipeline**
	rs2::pipeline pipe;
	...
	//**We make a variable myfile of type ofstream for making marix.txt file**
	std::ofstream myfile;
	...
	while (app)
	{
		...
		// **r is matrix , but actually its single array of size [12]**
		float r[12];
		...
		if (myfile.is_open())
		{
			std::cout<< "open file" << std::endl; //new line added
			myfile << "";

			// **To print 2 matrix for both images (right+left camera)**
			for (int i=0;i<2;i++)
			{
				for (int count = 0 ; count <12; count++)
				{

					std::cout<< "value at index" <<count << "is :"<< r[count] << std::endl ;
		...
		// Draw the trajectory from different perspectives on screen
